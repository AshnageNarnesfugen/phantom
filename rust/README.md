...
mismatch) to the in-app Transport Debugger. The native core does NOT yet handle
a real message — this is the on-device gate before any hot-path cutover.

### What remains

1. **Hot-path cutover** — route the app's real crypto through the native
   functions (behind the oracle), starting with the stateless ops, once the
   on-device parity log is confirmed green.
2. **Stateful ratchet over FFI** — `RatchetSession` needs opaque session
   handles + a `free` + length-prefixed buffers; better generated by
   `flutter_rust_bridge` than hand-written.
